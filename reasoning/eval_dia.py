...
class Dream(LM):
    def __init__(
        ...
        **kwargs,
        ...
        self.block_generation = kwargs.pop('block_generation',None)
        self.block_num = kwargs.pop('block_num',None)
        
        # Store latency information for each sample
    ...
    def _generate_batch(self, prompts: List[str]) -> Tuple[List[str], List[float]]:
        format_prompt = """
You are a helpful assistant that helps the user to solve the question. You need to think first and then answer the question briefly by following the format:<think>...</think><answer>...</answer>. 

Here is an example:
User: What is the answer of 1+2+3?
Assistant: <think> I need to add the numbers together. 1+2=3, 3+3=6. So the answer is 6.</think><answer> 6 </answer>

Here are the questions:

"""
        format_prompt_ids = self.tokenizer(format_prompt, return_tensors="pt").input_ids
        prompts = [format_prompt + p for p in prompts]
        ...
        format_ids = torch.tensor([self.tokenizer(f).input_ids for f in format],dtype = torch.int64).to(device="cuda")
        ...
        # Return responses with latency for each sample in batch
        latencies = [latency] * len(responses)  # Same latency for all samples in batch
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            # Measure latency for each sample in the batch
            batch_start_time = time.time()
            responses, latencies = self._generate_batch(contexts)
            ...
            # Store latency information directly in the request object
            for i, (req, latency) in enumerate(zip(batch_requests, latencies)):
                # Add latency as a custom attribute to the Instance
                req.generation_latency = latency
                # Also store in dict for backup
                req_key = (req.doc_id, req.idx)
                self._sample_latencies[req_key] = latency
    ...
    def get_latency_for_request(self, doc_id: int, idx: int) -> Optional[float]:
        """Get the stored latency for a specific request."""
        req_key = (doc_id, idx)
        return self._sample_latencies.get(req_key, None)


if __name__ == "__main__":
    # Monkey patch the evaluator to include latency in sample logs
    import lm_eval.evaluator as evaluator_module
    from lm_eval.evaluator import evaluate as original_evaluate
    
    def evaluate_with_latency(*args, **kwargs):
        # Get the lm model and log_samples flag
        lm = args[0] if args else kwargs.get('lm')
        log_samples_flag = kwargs.get('log_samples', True)
        
        # Call original evaluate
        result = original_evaluate(*args, **kwargs)
        
        # Add latency info to samples
        if log_samples_flag and 'samples' in result:
            for task_name, samples in result['samples'].items():
                eval_logger.info(f"Processing latency for task: {task_name}, {len(samples)} samples")
                for sample in samples:
                    # Get the corresponding Instance objects from task
                    # The evaluator stores requests which have latency info
                    # We need to match samples with their original requests
                    doc_id = sample.get('doc_id')
                    
                    # Try to get latency from model's stored dict
                    if hasattr(lm, '_sample_latencies'):
                        # Search for matching latency
                        for (stored_doc_id, idx), latency in lm._sample_latencies.items():
                            if stored_doc_id == doc_id:
                                sample['generation_latency_seconds'] = round(latency, 4)
                                break
        
        return result
    
    # Replace the evaluate function temporarily
    import lm_eval.evaluator
    lm_eval.evaluator.evaluate = evaluate_with_latency
